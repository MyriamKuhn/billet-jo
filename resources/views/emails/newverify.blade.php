...
<head>
    ...
    <title>{{ __('mail.title_email_update', ['app_name' => config('app.name')]) }}</title>
...
<body>
    <div class="email-container">
        ...
        <p>{{ __('mail.line_1_email_update') }}</p>
        <p>{{ __('mail.line_2_email_update') }}</p>
        <a href="{{ $url }}" class="button">{{ __('mail.action_email_update') }}</a>
        <p>{{ __('mail.note_email_update') }}</p>
        <p>{{ __('mail.thank_you') }}</p>
        <p>{{ __('mail.team', ['app_name' => env('APP_NAME')]) }}</p>
        <div class="footer">
            {{ __('mail.footer_email_update') }}<br>
            <a href="{{ $url }}">{{ $url }}</a>
        </div>
    </div>
</body>
